...
class FlutterineMenu extends StatefulWidget {
  ...
  FlutterineMenu(
      ...
      this.fabMargin = const EdgeInsets.all(5.0),
  ...
  @override
  FlutterineMenuState createState() => FlutterineMenuState();
}

class FlutterineMenuState extends State<FlutterineMenu>
  ...
  Widget _applyTransformations(Widget child, int index) {
    ...
    if (widget.alignment.x == 0) {
      angleFix = 1.0 * _directionY.abs();
    } else if (widget.alignment.y == 0) {
      angleFix = -1.0 * _directionX.abs();
    }

    final angle =
        vector.radians(180.0 / (widget.children.length - 1) * index + angleFix);
  ...
  void _calculateProps() {
    ...
    _ringDiameter =
        widget.ringDiameter ?? min(_screenWidth, _screenHeight) * 0.6;
    _ringWidth = widget.ringWidth ?? _ringDiameter! * 0.15;
